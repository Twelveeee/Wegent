...
vi.mock('@/api/dsh/desktopHost', () => ({ invokeDesktopHost: invoke, subscribeDesktopHostEvents: vi.fn(() => () => {}) }))
...
beforeEach(async () => {
  ...
  snapshot = { path: '/test/model.yml', revision: 'one', loadedAt: '', providers: [{ id: 'relay', name: 'My relay', base_url: 'https://relay.example/v1', api_format: 'openai-responses', api_key_configured: true, models: [] }] }
  invoke.mockReset()
...
describe('Provider settings page', () => {
  test('adds multiple models using the existing connection without re-entering a key', async () => {
    ...
    await user.type(screen.getByTestId('provider-batch-models'), 'upstream-a\nupstream-b\nupstream-a')
    await user.click(screen.getByTestId('provider-editor-save'))
    await waitFor(() => expect(invoke).toHaveBeenCalledWith('modelConfiguration.save', expect.anything()))
    ...
    expect(saved.providers[0].models.map((model: { model_id: string }) => model.model_id)).toEqual(['upstream-a', 'upstream-b'])
  ...
  test('a conflicting external edit keeps the unsaved form visible', async () => {
    ...
    invoke.mockImplementationOnce(async () => { throw new Error('model.yml changed externally. Reload before saving.') })
    await user.click(screen.getByTestId('provider-editor-save'))
    expect(await screen.findByTestId('provider-editor-error')).toHaveTextContent('changed externally')
