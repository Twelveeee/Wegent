...
describe('Provider configuration runtime adapter', () => {
  test('missing machine-local credentials disable only affected models and remain repairable', () => {
    const unavailable = resolveProviderModel({
      provider: { ...provider, api_key: undefined, api_key_ref: 'wework-model-key.missing' },
      model: { id: 'missing', model_id: 'a' },
    })
    ...
    const repaired = resolveProviderModel(
      { provider, model: { id: 'missing', model_id: 'a' } },
      unavailable
    )
