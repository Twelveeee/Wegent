...
import { findLocalModelConfigByModelName, listLocalModelConfigs, localModelName, markLocalModelCatalogReady, saveLocalModelConfig } from './localModelSettings'

beforeEach(() => { localStorage.clear(); replaceProviderModelConfigs([]) })
const provider = { id: 'relay', name: 'Relay', base_url: 'https://relay.example/v1', api_format: 'openai-responses' as const, api_key: 'test-key' }
...
describe('Provider configuration runtime adapter', () => {
  test('keeps existing local-model identity and never enters the Codex-provider namespace', () => {
    const model = resolveProviderModel({ provider, model: { id: 'stable-id', model_id: 'shared-upstream-id' } })
  ...
  test('changing credentials preserves catalog readiness but invalidates runtime identity version', () => {
    ...
    const after = resolveProviderModel({ ...entry, provider: { ...provider, api_key: 'new-key' } }, before)
  ...
  test('protocol overrides inherit the key but not an incompatible request path', () => {
    const model = resolveProviderModel({ provider: { ...provider, request_path: '/responses' }, model: { id: 'm', model_id: 'upstream', api_format: 'anthropic-messages' } })
  ...
  test('migration groups only identical connections and preserves capabilities and IDs', () => {
    const first = saveLocalModelConfig({ id: 'legacy-a', modelId: 'a', baseUrl: provider.base_url, apiKey: 'key-a', contextWindow: 12345 })
    const second = saveLocalModelConfig({ id: 'legacy-b', modelId: 'b', baseUrl: provider.base_url, apiKey: 'key-a' })
    const third = saveLocalModelConfig({ id: 'legacy-c', modelId: 'a', baseUrl: provider.base_url, apiKey: 'different-key' })
  ...
  test('projection shadows only matching migrated IDs; legacy records and cloud-looking names remain distinct', () => {
    const legacy = saveLocalModelConfig({ id: 'old', modelId: 'shared', baseUrl: provider.base_url })
    saveLocalModelConfig({ id: 'independent', modelId: 'shared', baseUrl: provider.base_url })
    const migrated = resolveProviderModel({ provider, model: { id: legacy.id, model_id: legacy.modelId } })
    ...
    expect(() => saveLocalModelConfig({ id: 'old', modelId: 'changed', baseUrl: provider.base_url })).toThrow(/Provider settings/)
  })
})
