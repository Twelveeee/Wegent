...
- Keep model options the user changed explicitly when a scope restores its selection config again. A refreshed runtime task snapshot only echoes what was sent to the runtime, so it must not drop a pending composer choice such as plan mode.
