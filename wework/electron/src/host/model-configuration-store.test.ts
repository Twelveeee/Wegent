...
describe('Provider configuration file store', () => {
  test('a missing credential reference does not block the settings page or other models', async () => {
    await writeFile(
      join(directory, 'model.yml'),
      YAML.replace('api_key: secret-not-for-output', 'api_key_ref: wework-model-key.missing')
    )
    ...
    await store.save(
      snapshot.revision,
      snapshot.providers.map(provider => ({ ...provider, api_key: 'replacement' }))
    )
